...
import SellCarDrawer from "@/app/(main)/inventory/components/SellCarDrawer"
import { getUserLevelAction } from "@/app/actions/userActions"
import { createClient } from "@/app/utils/supabase/client"
import { deleteCarByIdAction } from "@/app/actions/carActions"
import { FaEdit, FaTrash } from "react-icons/fa"

/**
 * Props for the DashboardCarCard component
 * Displays a car card with details and actions
 */
interface DashboardCarCard {
    ...
    kilometer: number;
    /** Current status (e.g., 'Tersedia', 'Terjual') */
    status: string;
...
export default function DashboardCarCard(props: DashboardCarCard) {
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this car?')) {
            setIsDeleting(true);
            try {
                const result = await deleteCarByIdAction(props.id);
                if (result.success) {
    ...
    return (
        <>
            ...
            <SellCarDrawer
                ...
                car={{
                    id: props.id,
                    title: `${props.merk} ${props.model} ${props.series}`,
                    price: props.harga_jual,
